...
export function BillingContent({ showTitle = true }: { showTitle?: boolean }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchBillingData = async () => {
      ...
      if (pkgData) setActivePackage(pkgData as any);
    ...
    return (
      <div className="space-y-6">
        <div className="animate-pulse space-y-4">
          {showTitle && <div className="h-8 bg-muted rounded w-1/3" />}
                ...
                return (
                  ...
                  >
                    <CardContent className="pt-6 text-center space-y-3">
                      ...
                      {savings > 0 && <p className="text-xs text-primary">You save ${savings.toFixed(0)}</p>}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-foreground">{invoice.packages?.name || "Package"}</p>
                        <p className="text-sm text-muted-foreground">
